import React from 'react'
import Settings from './settings'
import mui, {RaisedButton, Toggle, FloatingActionButton, FontIcon} from 'material-ui'
import FixedDataTable, {Table, Column, ColumnGroup as Group} from 'fixed-data-table'

let DataTable = React.createClass({

  propTypes: {
    colNames: React.PropTypes.array,
    data: React.PropTypes.array,
    colWidths : React.PropTypes.array,
    maxWidth: React.PropTypes.number
  },

  rowGetter: function(rowIndex) {
      return this.props.data[rowIndex];
  },

  getWidth: function() {
    let width = window.innerWidth - Settings.leftNavWidth - (2 * Settings.contentPadding) - Settings.widthBuffer;
    return width;
  },

  getColWidth: function(i) {
    if(this.props.colWidths) {
      return (this.state.width * (this.props.colWidths[i] / this.props.maxWidth));
    } else {
      return (this.state.width / this.props.colNames.length);
    }
  },

  handleResize: function() {
    this.setState({width: this.getWidth()});
  },

  componentDidMount: function() {
    window.addEventListener('resize', this.handleResize);
  },

  getInitialState: function() {
    return { width: this.getWidth(), active: '' };
  },

  style: function() {
    return {
      width: this.state.width + "px",
      margin: "20px 0"
    };
  },

  onRowClick: function(e, index) {
      this.setState({ active: index });
  },

  getRowClass: function(index) {

      var active = this.state.active;

      if( index === active ) {
        return 'active';
      }

      return;
  },

  formatDate(c) {
    if(c) {
      var d = new Date(c);
      c = d.toLocaleDateString();
    }

    return c;
  },

  formatCell: function(cell, col) {

    var type = col.type || 'string';

    if(cell !== '') {

      // Date
      if(type === 'date') {
        return this.formatDate(cell);
      }

      // Currency
      else if(type === 'currency') {
        return '$ ' + cell;
      }

      // User
      else if(type === 'user') {
			return <a href="#" style={{ color: '#333'}}><span className="md md-account-box" style={{ fontSize: '1.4em', verticalAlign: 'text-bottom' }}></span> {cell}</a>;
      }

      // Button
      else if(type === 'button') {
        return <div style={{textAlign: 'center'}} ><RaisedButton label={cell} /></div>;
      }
			
			// Send
			
			else if(type === 'send') {
        return <div style={{textAlign: 'center'}} ><RaisedButton><FontIcon className="md-send button-icon" /><span className="button-label">{cell}</span></RaisedButton></div>;
			}

      // Link
      else if(type === 'link') {
				return <a href='#' className="c-p-1">{cell}</a>;
      }
			
			// Toggle
			else if(type === 'toggle') {
				if(cell === "Active") {
					var status = <div className="c-green"><span className="md md-check"></span> Active</div>;
				} else {
					var status = false;
				}
				return <div className="c-a-1"><span className="md md-close"></span> Inactive</div>;
			}
    }

    return cell;
  },

  render: function() {
    let columns =
      <Group fixed={true}>
        {
          this.props.colNames.map((col, i) =>
            <Column
              label={col.name}
              dataKey={i}
              width={this.getColWidth(i)}
              cellRenderer={
                function(cellData) {
                  return this.formatCell(cellData, col);
                }.bind(this)
              }/>
          , this)
        }
      </Group>;

    return (
      <div style={this.style()} className="table">
        <Table
          rowHeight={50}
          onRowClick={this.onRowClick}
          rowGetter={this.rowGetter}
          rowsCount={this.props.data.length}
          rowClassNameGetter = {this.getRowClass}
          width={this.state.width}
          height={window.innerHeight - 300}
          headerHeight={50}>
            {columns}
        </Table>
      </div>
    );
  }
});

export default DataTable;
